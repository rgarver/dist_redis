...
var RedisArray = exports.RedisArray = function(str) {
	this.rawString = str;
	this.tokens = [];
}
...
RedisArray.prototype.command = function() {
	var tokenizer = new tok.Tokenizer(this.rawString);
	if(tokenizer.peek() == "*") {
		tokenizer.nextUntil("\r"); tokenizer.next(2); // Line 1
		tokenizer.nextUntil("\r"); tokenizer.next(2); // Line 2
	}
	var cmd = tokenizer.nextUntil(/[ ]|\r/).toUpperCase();
...
var validRedisResponse = exports.validRedisResponse = function(str) {
	var tokenizer = new tok.Tokenizer(str);
	switch(tokenizer.next()) {
		case "-":
		case "+":
			if(tokenizer.nextUntil("\r").length = 0) return false;
			if(tokenizer.next(2) != "\r\n") return false;
			break;
		case "$":
			var len = parseInt(tokenizer.nextUntil("\r"));
			if(_(len).isNaN()) return false;
			if(tokenizer.next(2) != "\r\n") return false;
			if(tokenizer.next(len).length != len) return false;
			break;
		case "*":
			var arrLen = parseInt(tokenizer.nextUntil("\r"));
			if(_(arrLen).isNaN()) return false;
			if(tokenizer.next(2) != "\r\n") return false;
			for(var i=0; i < arrLen; i++) {
				if(tokenizer.next() != "$") return false;
				var len = parseInt(tokenizer.nextUntil("\r"));
				if(_(len).isNaN()) return false;
				if(tokenizer.next(2) != "\r\n") return false;
				if(len >= 0) {
			...
			break;
		case ":":
			if(_(parseInt(tokenizer.nextUntil("\r"))).isNaN()) return false;
			if(tokenizer.next(2) != "\r\n") return false;
			break;
		default:
			return false;
